...
export default function RegisterVerifyScreen(): React.JSX.Element {
  ...
  const handleConfirm = async (): Promise<void> => {
    ...
    if (!verificationId) {
      Alert.alert(t.common.error, t.registerVerify.genericError);
      return;
    }
    if (!code.trim() || code.trim().length !== 6) {
      Alert.alert(t.common.error, t.registerVerify.invalidCode);
      return;
    }
    ...
    try {
      ...
      Alert.alert(t.common.error, error?.message || t.registerVerify.genericError);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleResend = async (): Promise<void> => {
    ...
    try {
      setResending(true);
      await authService.registerResend({ verificationId });
      Alert.alert(t.common.success, t.registerVerify.resendSuccess);
    } catch (error: any) {
      Alert.alert(t.common.error, error?.message || t.registerVerify.genericError);
    } finally {
      setResending(false);
  ...
  return (
    <View style={styles.container}>
      ...
      >
        ...
        <Text style={styles.headerTitle}>{t.registerVerify.verifyTitle}</Text>
        <View style={{ width: 40 }} />
      </LinearGradient>
      ...
      >
        <View style={styles.iconSection}>
          ...
          <Text style={styles.iconTitle}>{t.registerVerify.verifyTitle}</Text>
          <Text style={styles.iconSubtitle}>{t.registerVerify.verifySubtitle}</Text>
        </View>
        ...
        >
          {resending
            ...
            : <Text style={styles.resendText}>{t.registerVerify.resendButton}</Text>
          }
        </TouchableOpacity>
          ...
          >
            {loading ? (
              ...
              <>
                ...
                <Text style={styles.primaryButtonText}>{t.registerVerify.confirmButton}</Text>
              </>
            )}
          </LinearGradient>
        </TouchableOpacity>
